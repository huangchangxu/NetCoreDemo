...
using PaymentCenter.PaymentCore.Alipay.Parser;
using PaymentCenter.PaymentCore.Alipay.Request;
using PaymentCenter.PaymentCore.Alipay.Util;
using System.Text;
using System.Web;
...
#pragma warning disable CS0169 // 从不使用字段“DefaultAopClient.httpmethod”
        private string httpmethod;
#pragma warning restore CS0169 // 从不使用字段“DefaultAopClient.httpmethod”
            ...
            else
            {
                ...
                if (reqMethod.Equals("GET"))
                {
                    ...
                    if (txtParams != null && txtParams.Count > 0)
                    {
                        if (tmpUrl.Contains("?"))
                        {
                            tmpUrl = tmpUrl + "&" + PaymentCenter.PaymentCore.Alipay.Util.WebUtils.BuildQuery(txtParams, charset);
                        }
                        else
                        {
                            tmpUrl = tmpUrl + "?" + PaymentCenter.PaymentCore.Alipay.Util.WebUtils.BuildQuery(txtParams, charset);
