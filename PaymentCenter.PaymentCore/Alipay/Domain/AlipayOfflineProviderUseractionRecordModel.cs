...
namespace PaymentCenter.PaymentCore.Alipay.Domain
{
    ...
    {
        /// <summary>
        /// 详情设置会根据action_type字段类型不同而格式不同，请详细查看开放平台文案，会详细说明如果设置，整体是json结构。订单数据回流详细说明见链接：https://doc.open.alipay.com/docs/doc.htm?spm=a219a.7629140.0.0.msmB7o&treeId=193&articleId=106810&docType=1#s1
        /// </summary>
        ...
        /// <summary>
        /// 上传类型为：order_dishes(上传用户菜单)、order_num(餐厅排号)、order_book_create(餐厅预定）设置的类型都是REPAST；  上传类型为：order_pan(泛行业订单）设置的类型是PAN。
        /// </summary>
        [XmlElement("industry")]
        ...
        /// <summary>
        /// 该字段建议填写。值定义：alipay、weixin、other、isv;  值意义：alipay：支付宝；weixin：微信；isv：isv自己的系统未通过第三方点菜，other：其他;  当前订单的创建来源，比如支付宝扫码创建或微信扫码创建或通过自己的系统用户点菜后创建，则传入对应英文。
        /// </summary>
