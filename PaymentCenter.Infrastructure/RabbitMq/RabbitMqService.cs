...
namespace PaymentCenter.Infrastructure.RabbitMq
{
    ...
    public class RabbitMqService : IDisposable
    {
        ...
        /// <returns></returns>
        private static IModel GetModel(string exchange, string queue, string routingKey, bool isProperties = false, string type = ExchangeType.Direct)
        {
        ...
        public void Subscribe<T>(Func<T,bool> handler) where T : class
        {
            var queueInfo = GetRabbitMqAttribute<T>();
            if (queueInfo.IsNull())
                throw new ArgumentException(RabbitMqAttribute);
        ...
        public void Subscribe<T>(string queue, bool isProperties, Func<T,bool> handler, bool isDeadLetter) where T : class
        {
            //队列声明
