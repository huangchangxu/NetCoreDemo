...
namespace PaymentCenter.Infrastructure.ConfigCenter
{
    ...
    public class ApplicationConfigHelper
    {
        ...
        private static readonly string _cacheKey="config";
        private static Dictionary<string, dynamic> _configurationCache = new Dictionary<string, dynamic>();
        ...
        /// 配置节点关键字
        /// </summary>
        private static string _configSection = "compilerOptions";
        ...
        static ApplicationConfigHelper()
        {
        ...
        private static void ConfigFinder(string Path)
        {
            ...
            if (config_info.Exists)
            {
                ...
                throw new FileNotFoundException($"配置文件不存在或者路径不正确！检索目录为【{_defaultPath}】");
        }
        ...
        private static void LoadConfiguration()
        {
            ...
            dynamic config_dynamic = LoadJson(_configPath);
            if (config_dynamic == null || !(config_dynamic is JObject)) return;
            ...
            if (_configurationCache.ContainsKey(_cacheKey))
                _configurationCache[_cacheKey] = config_dynamic;
            else
                _configurationCache.Add(_cacheKey, config_dynamic);
        }
        ...
        /// <returns></returns>
        private static dynamic LoadJson(string filePath)
        {
            dynamic config_object = null;
            try
            {
                StreamReader streamReader = new StreamReader(filePath, Encoding.Default);
                config_object = JObject.Parse(streamReader.ReadToEnd());
                streamReader.Close();
            }
            catch { }
            return config_object;
        ...
        {
            get { return _configurationCache[_cacheKey]; }
        }
